...
const initialState = {
  isLoading: false, // fetchをしてる最中かどうか
  isLoginView: true, // LoginとRegisterの画面を切り替えるときにtrueならLogin
  error: "", // errorの文字列を保有するstate
  credentialsLog: {
    // 認証の情報を保有するstate
    email: "",
    password: "",
  ...
  const login = async (event) => {
    ...
    if (state.isLoginView) {
      try {
        ...
        // const resよりしたの動作を待つためにawaitを使用し同期にする。
      ...
      <form onSubmit={login}>
        <div className={classes.paper}>
          ...
          <TextField
            ...
            type="password"
          />
          <span className={classes.spanError}>{state.errer}</span>
